...
\highlight{$p(\mathbf{x}) = \prod_{i=1}^n p(x_i | \mathbf{x}_{<i})$}
...
\highlight{$\mathbf{Y} = \text{softmax}\left(\frac{\mathbf{X}\mathbf{W}_Q (\mathbf{X}\mathbf{W}_K)^T}{\sqrt{D}} + \mathbf{M}\right) \mathbf{X}\mathbf{W}_V$}
